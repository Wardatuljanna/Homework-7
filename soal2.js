...
// Fungsi untuk menulis log ke dalam file log.txt
fs.writeFile('log.txt', logData, (err) => {
  if (err) {
    console.error('Kesalahan penulisan ke log.txt:', err);
  } else {
    console.log('Log data berhasil dicetak ke log.txt\n');
  }
});

// Fungsi untuk membaca isi file log.txt
fs.readFile('log.txt', 'utf8', (err, logData) => {
  if (err) {
    console.error('Kesalahan membaca log.txt:', err);
  } else {
    console.log('Berikut isi dari log.txt:\n', logData);
  }
});